...
@push('scripts')
    <script>
        function validasi(soalUjianId) {
            ...
            $('#modalValidasiUjian').modal('show');
        }

        function submitValidasi() {
    var formData = new FormData(document.getElementById('formValidasiUjian'));

    $.ajax({
        url: '{{ route("soalUjian.validasi") }}',  // Sesuaikan dengan route untuk validasi
        method: 'POST',
        data: formData,
        processData: false,
        contentType: false,
        success: function(response) {
            ...
            Swal.fire({
                title: "Sukses!",
                text: "Soal ujian berhasil divalidasi.",
                icon: "success",
                confirmButtonText: "OK"
            }).then(() => {
                $('#modalValidasiUjian').modal('hide');
                location.reload();  // Refresh halaman
            });
        },
        error: function(xhr) {
            let errorMessage = "Terjadi kesalahan saat memvalidasi!";
            Swal.fire({
                title: "Error!",
                text: errorMessage,
                icon: "error",
