                ...
                <table id="kt_datatable_fixed_header" class="table table-striped table-row-bordered gy-5 gs-7">
                    ...
                    <tbody>
                        @foreach ($prodis as $index => $prodi)
                            <tr>
                                <td>{{ $index+1 }}</td>
                                <td>{{ $prodi->kode_user }}</td>
                                <td>{{ $prodi->nidn }}</td>
                                <td>{{ $prodi->nama_lengkap }}</td>
                                <td>{{ $prodi->email }}</td>
                                <td>{{ $prodi->prodi->nama_prodi }}</td>
                                <td>
                                    <div class="d-flex justify-content-between gap-2">
                                        <button type="button" class="btn btn-sm btn-primary" onclick="editDosen({{ $prodi->id }})">Edit</button>
                                        <button type="button" class="btn btn-sm btn-danger" onclick="hapusDosen({{ $prodi->id }})">Hapus</button>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
